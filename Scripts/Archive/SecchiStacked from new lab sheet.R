...
folder_path <- "/Users/mevanskeene/Desktop/Project Coast/2025/Coast Data April 2025/Data/CHL"
...
read_as_text <- function(file) {
  col_names <- read_excel(file, sheet = "Depth, Secchi Disk Depth", skip = 1, n_max = 1) %>% names()
  
  
  # Read the entire sheet with all columns as text
  df <- read_excel(file, sheet = "Depth, Secchi Disk Depth", skip = 1, col_types = rep("text", length(col_names)))

  # Convert the activity time column 
  df$Activity_Start_Time <- as.character(df$Activity_Start_Time)  # Ensure time is a character
  
  # If time is stored as numeric (fraction of a day), convert it back to time format
  if(any(grepl("^[0-9.]+$", df$Activity_Start_Time))) {  # Check if the column has numbers
    df$Activity_Start_Time <- as.numeric(df$Activity_Start_Time) * 86400  # Convert to seconds
    df$Activity_Start_Time <- format(as.POSIXct(df$Activity_Start_Time, origin = "1970-01-01", tz = "UTC"), "%I:%M:%S %p")
  }
  ...
  df$Total_Depth <- as.numeric(df$Total_Depth)
  df$Total_Depth <- round(df$Total_Depth, 2)
...
write.table(combined_data, file = "/Users/mevanskeene/Documents/GitHub/WIN File Creation/Data/Project Coast/Secchi_STACKED.csv", sep = ",", na = "", row.names = FALSE, quote = FALSE)
