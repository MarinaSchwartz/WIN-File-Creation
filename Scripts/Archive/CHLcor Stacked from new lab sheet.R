...
read_as_text <- function(file) {
  col_names <- read_excel(file, sheet = "Chlorophyll a- corrected", skip = 1, n_max = 1) %>% names()

  
  # Read the entire sheet with all columns as text
  df <- read_excel(file, sheet = "Chlorophyll a- corrected", skip = 1, col_types = rep("text", length(col_names)))
...
write.table(combined_data, file = "/Users/mevanskeene/Documents/GitHub/WIN File Creation/Data/Project Coast/CHLcor_STACKED.csv", sep = ",", na = "", row.names = FALSE, quote = FALSE)
